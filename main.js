...
const actualizarCarrito = () => {
  ...
  let cantidadTotal = data.reduce((acc, prod) => acc + prod.cantidad, 0);
  iconoCarrito.innerHTML = `${cantidadTotal}`;
  let precioTotal = data.reduce(
...
const eliminarDelCarrito = (prodId) => {
  ...
  item.cantidad > 1 ? item.cantidad-- : miCarrito.productos.splice(indice, 1);
  actualizarCarrito();
};
